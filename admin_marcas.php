...
//Query para la tabla de marcas
$q_marcas = ("SELECT id_marca,marca_descripcion, (SELECT count(id_producto) FROM productos WHERE id_marca = c_marcas.id_marca) AS total FROM c_marcas");
...
$t_marcas = pg_query($conn, $q_marcas);
  ...
  <div class="col">
    <div class="row-fluid" style="background-color: #FFFFFF">
      <div class="col" style="margin: 5px">
        ...
        echo '<h4>Eliminar marca</h4>';
        //Revisar si hay marcas
        $hay_marca = pg_num_rows($t_marcas);
        if ($hay_marca < 1) {
          echo '<h5>No hay marcas registradas</h5>';
        } else {
          //Generar tabla de marcas
          echo '<table class="table-bordered"><tr><th>ID</th><th>Nombre</th><th>Productos</th></tr>';
          while ($row = pg_fetch_row($t_marcas)) {
            echo '<tr><td>'.$row[0].'</td><td>'.$row[1].'</td><td>'.$row[2].'</td>';
            if ($row[2] > 0) {
              echo '</tr>';
            } else {
              echo '<td><form action="del_marca.php" method="post"><button name="marca_id" value="'.$row[0].'">Eliminar</button></form></td></tr>';
            }
          }
          echo '</table>';
        }
