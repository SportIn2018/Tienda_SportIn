  ...
  <div class="col" style="background-color: #FFFFFF">
      <?php
        if ($_SESSION['prod_msg']) {
          switch ($_SESSION['prod_msg']) {
            case 1:
              echo '<div class="row" style="margin: 2px">No se pudo agregar el producto: El precio tiene que ser positivo y mayor a cero</div>';
              unset($_SESSION['prod_msg']);
              break;
            case 2:
              echo '<div class="row" style="margin: 2px">No se pudo agregar el producto: La cantidad inicial tiene que ser positiva y mayor a cero</div>';
              unset($_SESSION['prod_msg']);
              break;
            case 3:
              echo '<div class="row" style="margin: 2px"><div class="row" style="margin: 2px">No se pudo agregar el producto: Hubo un error con la base de datos, intentelo mas tarde</div>';
              unset($_SESSION['prod_msg']);
              break;
            case 4:
              echo '<div class="row" style="margin: 2px">El producto se ha agregado exitosamente</div>';
              unset($_SESSION['prod_msg']);
              break;
          }
        }
    ...
    <div class="row" style="margin: 2px">
      ...
      <?php
        ...
        //Revisar si hay categorias y marcas para poder agregar producto
        $hay_categ = pg_num_rows($process);
        $hay_marca = pg_num_rows($marca);
        if ($hay_categ > 0 AND $hay_marca > 0) {
      ?>
    </div><div class="row" style="margin: 2px">
      <form enctype="multipart/form-data" action="producto_procesos.php" method="post">
      <!--Indicar agregar producto-->
      <input type="text" style="display:none" name="producto" value="1">
      ...
      Imagen: <input type="file" name="file"></input><br><br>
      Imagen Banner: <input type="file" name="file_banner"></input><br><br>

      <input type="submit" value="Enviar">
      </form>
      <?php
        } else {
          echo '<h4>Para poder agregar un nuevo producto primero agregue al menos una categoria y marca</h4>';
        }
      ?>
    </div>
  </div>
